...
-- Now we'll create necessary tables
...
select 
	...
	SUBSTRING(Name, CHARINDEX(',', Name)+1, len(Name)-CHARINDEX(',', Name)) as SecondName
...
select
	T.Vals, S.Value
from Splitter T
cross apply string_split(T.Vals, '.') as S
...
where Salary > (select salary from Employee c where t.ManagerId=c.Id)
-- solution with join
SELECT 
    e.Id AS EmployeeId,
...
select * from weather t1
join weather t2
on t1.Id=t2.id+1 and t1.Temperature>t2.Temperature

--Write an SQL query that reports the first login date for each player.(Activity)
select distinct player_id, min(event_date) as [First Login Date] from Activity
group by player_id

--Your task is to return the third item from that list.(fruits)
select 
	SUBSTRING(fruit_list, charindex(',', fruit_list, charindex(',', fruit_list)+1)+1, charindex(',', fruit_list, charindex(',', fruit_list)+1)-charindex(',', fruit_list)-1),
	fruit_list
from fruits
...
select 
	...
	DATEDIFF(YEAR, HIRE_DATE, getdate()) as [Yeard of Service],
	case
...
select 
	Vals,
	substring(Vals, charindex(',', Vals)+1, charindex(',', Vals, charindex(',', Vals)+1)-charindex(',', Vals)-1) + ',' + left(Vals, charindex(',', Vals)-1) + ','+ substring(Vals, charindex(',', Vals, charindex(',', Vals, charindex(',', Vals)+1))+1, len(Vals)) as Reversed
from MultipleVals
...
declare @i int = 1
create table string1 (stringlar varchar(50))
select 'sdgfhsdgfhs@121313131' as [Berilgan string]

while @i<=len(@s)
begin
	insert into string1(stringlar) values (substring(@s,@i,1))
	set @i = @i + 1
end
select * from string1
drop table string1
...
select player_id, device_id from Activity p
where event_date = (
...
declare @i int = 1 
select 'rtcfvty34redt' as [Berilgan string]
create table Table1 (Harflar varchar(50), Raqamlar int)

while @i<=len(@str)
begin
	if substring(@str,@i,1) like '[0-9]' 
		insert into Table1(Raqamlar) 
		values (substring(@str,@i,1));
	else 
		insert into Table1(Harflar) values (substring(@str,@i,1));
	set @i += 1;
end
