...
select sum(SaleAmount) as [Total Amount] from Sales
where ProductID = 7
select * from Sales
...
select DepartmentName, count(EmployeeID) as [Number of emp] from Employees
group by(DepartmentName)

--Write a query to show the minimum and maximum Price of products grouped by Category. Use products table.
select Category, min(price) as [min price], max(price) as [max price] from Products
group by Category
...
select DepartmentName, count(EmployeeID) as [Num of emp] from Employees
group by(DepartmentName)
having COUNT(EmployeeID)>5

-- Task2 
--Write a query to calculate the total sales and average sales for each product category from the Sales table.
select ProductID, sum(SaleAmount) [Total], avg(SaleAmount) as [Average] from Sales
group by ProductID
...
select DepartmentName, min(salary) as [min salary], max(salary) as [max salary] from Employees
group by DepartmentName

--Write a query to calculate the average salary per Department.(DeptID is enough, if you don't have DeptName).
select DepartmentName, avg(salary) as [avg salary] from Employees
group by DepartmentName
...
select DepartmentName, avg(salary) as [avg salary], count(EmployeeID) as [num of emp] from Employees
group by DepartmentName
...
select year(SaleDate) as years, sum(SaleAmount) as [Total] from Sales
group by year(saledate)

--Write a query to show the list of customers who placed at least 3 orders.
select CustomerID, count(OrderID) as [Num of orders] from Orders
group by CustomerID
...
select month(saledate) as [months], year(saledate) as [years], sum(saleamount) as [Total Sales], count(distinct ProductID) as UniqueProducts from Sales
...
select year(orderdate) as years, min(quantity) as min_q, max(quantity) as max_q from Orders
