...
select Category, count(*) as [Num of Products] from Products
group by(Category)
...
case
	when StockQuantity < 30 then 'Low Stock' else 'Sufficient' end as Label
from Products

--Using Customers table, find the total number of customers in each country.
select Country, count(*) as NumOfCustomers from Customers
group by Country
...
and o.CustomerID not in (select CustomerID from Invoices where year(InvoiceDate) = 2023 and month(CustomerID) = 1)
...
select year(OrderDate) as Years, avg(TotalAmount) as Average from Orders
...
select district_name, [2012], [2013] into Population_Each_Year_Table
from (select district_name, population, year from city_population) as src
pivot(
	sum(population)
...
select district_name, population, year into org_table
from (select district_name, [2012],[2013] from Population_Each_Year_Table) as src
unpivot (
...
select ProductName, count(s.SaleID) as [Sold Num] from Products p
join Sales as s
on 
...
select year, city, population into org_table1
from (select year, [Bektemir],[Chilonzor],[Yakkasaroy] from Population_Each_City) as src
unpivot (
