...
select concat(cast(employee_id as varchar(10)), '-', FIRST_NAME, ' ', LAST_NAME) from Employees
where EMPLOYEE_ID = 100
-- another method without concat
select cast(employee_id as varchar(10))+'-'+first_name+' '+last_name from Employees
where Employee_id = 100
...
select
	year1,
	greatest(max1,max2,max3) as MaximumNumber
from TestMax
...
select 
	coalesce(id, ssn, passportid, itin) as FirstNonNull
from person
...
select
	...
	SUBSTRING(FullName, charindex(' ', FullName,(charindex(' ', FullName)+1)), len(FullName)+1-charindex(' ', FullName,(charindex(' ', FullName)+1))) as LastName
from Students
...
select 
	STRING_AGG(String, ' ') as ConcatenedValues
from DMLTable
...
select 
	DEPARTMENT_ID, 
	COUNT(EMPLOYEE_ID) as TotalOfEmp,
	cast(sum(case 
				when datediff(year, hire_date, getdate()) >= 3 then 1 
				else 0 end)
				/count(employee_id) as decimal(5,2)
				) as Percentage
from Employees
...
select 
	c.StudentID,
	c.Grade + isnull(p.Grade, 0) as SumWithPrevious
from Students c
left join Students p
...
declare @other nvarchar(100) = '';

while @i <= @len
begin
	set @ch = SUBSTRING(@text, @i, 1)

	if @ch COLLATE SQL_Latin1_General_CP1_CS_AS between 'A' and 'Z'
		set @upper += @ch;
	else if @ch COLLATE SQL_Latin1_General_CP1_CS_AS between 'a' and 'z'
		set @lower += @ch;
...
select @upper as UpperCase, @lower as LowerCase, @number as Numbers, @other as OtherCharacters
