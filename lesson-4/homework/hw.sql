...
select FirstName, LastName, coalesce (FirstName, LastName) as NotNullNAme from Employees
...
select FirstName + ' ' + LastName as FullName from Employees --  this is 1st method, but if one of the values is null, it returns null.

select concat(FirstName,' ',LastName) from Employees -- in this 2nd method we used concat command to add strings, it ignores null values
